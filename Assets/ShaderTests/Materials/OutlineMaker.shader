    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 getScaledDepthNormals(float2 uv){
                ...
                float2 scaledDepth = Linear01Depth( max( sampleDepthNormals.b, sampleDepthNormals.b ));



                return float4(scaledNormal.rgb, -scaledDepth.x);
            
            }


            float3 sobelDepthConvolution (float2 uv){
                float2 ts = _MainTex_TexelSize.xy;
                float3 result = 0;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float3 depthMagnitude = sobelDepthConvolution(i.uv);
